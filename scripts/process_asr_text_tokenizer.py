...
#
# python process_asr_text_tokenizer.py --manifest="train_clean_100,train_clean_360,train_other_500" \
#         --data_root="<output directory>" \
#         --vocab_size=<number of tokens in vocabulary> \
...
#
# Args:
#   --manifest or --data_file: If your text data lies inside an ASR manifest file,
...
#       English language). For almost all languages with small base token sets (<1000 tokens),
#       this should be kept at its default of 1.0. For languages with larger vocabularies (say Japanese,
...
#
#   --spe_max_sentencepiece_length: Limits the maximum length that any SentencePiece subword can be.
#       Using this will change the subword tokens generated.
#
#   --spe_pad: Adds <pad> as special token.
#
#   --spe_bos: Adds <s> as Beginning-of-Sentence special token.
#
...
parser.add_argument(
    "--tokenizer", default="wpe", choices=["spe", "wpe"], help='Type of tokenization to perform'
)
parser.add_argument(
    "--spe_type",
    default="bpe",
...
parser.add_argument(
    '--spe_train_extremely_large_corpus', action='store_true', help='Train extremely large corpus'
)
parser.add_argument(
...
def __process_data(
    ...
    """
    Converts text to tokenizer model.

    Args:
        text_path: source with text lines
        dst_folder: where tokenizer model files will be stored
        vocab_size: vocabulary size used in encoding the text
        ...
        spe_sample_size: int, default of -1. If positive integer is used, samples the dataset by given sample size.
        spe_train_extremely_large_corpus: bool. If dataset is too large, and user has sufficient RAM,
            this flag can be set to try to train the tokenizer. Will silently fail if it runs out of RAM.
    ...
    Returns:
        tokenizer_path: path to the created tokenizer.
    """
