...
# Provide a minimal nv_one_logger stub so NeMo imports succeed when the
# vendor package is not available in this environment. This creates the
# modules and attributes that NeMo checks at import-time (no-op implementations).
if 'nv_one_logger' not in sys.modules:
    try:
        import nv_one_logger  # type: ignore
    except Exception:
        ...
        config_mod.TrainingTelemetryConfig = TrainingTelemetryConfig
        api_mod.config = config_mod
        mod.api = api_mod

        # training_telemetry.api.callbacks.on_app_start no-op
